...
/*
    ...
    destino es un vértice del grafo al que tenemos que llegar desde cualquier punto del grafo

    P es un vector de vértices que indica el camino mínimo, guardando el vértice anterior a cada uno del grafo

    D es la distancia mínima hasta un punto

    S representa el conjunto de vértices que ya han sido usados para mejorar el camino
...
*/
{
    ...
    //Iniciamos D y P con caminos directos HASTA destino

//    Esto hay que cambiarlo para que pille la columna
    D = vector<tCoste>(n, GrafoP<tCoste>::INFINITO);
    for(vertice v = 0; v < n ; v++)
            D[v]= G[v][destino];

    //Coste destino-destino es 0
    D[destino] = 0;
    P = vector<vertice>(n,destino);

    //Calculamos caminos de coste mínimo
    S[destino] = true;                          // Incluir vÃ©rtice origen en S.
    for (size_t i = 1; i <= n-2; i++) {
          // Seleccionar vÃ©rtice w no incluido en S
          // con menor coste desde origen.
          tCoste costeMin = GrafoP<tCoste>::INFINITO;
        for (v = 0; v < n; v++)
            if (!S[v] && D[v] <= costeMin) {
          ...
          S[w] = true;                          // Incluir vÃ©rtice w en S.
          // Recalcular coste desde cada v no incluido en S a travÃ©s de w.
          for (v = 0; v < n; v++)
            if (!S[v]) {
